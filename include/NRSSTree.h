...
public :
   ...
   virtual void ResetNRSS();
   virtual void Store(CAEN_DGTZ_X743_EVENT_t *x5743Event, double timestamp, int nev, int nrun);
...
//RESET Tree
///////////////////////////////////////////////////////////////
void  NRSSEvTree::ResetNRSS() {

  for(Int_t i=0; i<8; i++) {  //loop su canali
	IsChannelActive[i] = 0;
	baseline[i] = 0;
	n_raw_maxima[i] = 0; 
	RawS[i]->Clear();
...
void  NRSSEvTree::CreateTree(TString rootfile) {
 ...
 // myTree->Branch("TrigTimeReference",TrigTimeReference,"TrigTimeReference[8]/D");
  ...
  //Define Raw Spectra
  //chname[8] = {"1A","1B","2A","2B","3A","3B","4","LYSO"};
  for (int i = 0; i < 8; ++i){
  specname[i].Form("RawWaveForm_%d",i);
...
//Store Tree
void NRSSEvTree::Store(CAEN_DGTZ_X743_EVENT_t *x5743Event, double timestamp, int nev, int nrun) {

TimeStamp = timestamp;
nEv = nev;
nRun = nrun;

	printf("======= Tree storing routine =======\n");
	printf("TimeStamp = %i \n",timestamp);
	printf("NEV = %i \n",nEv);
	printf("nRun = %i \n",nRun);
...
{
	int gridx = i/2;
	IsChannelActive[i] = x5743Event->GrPresent[gridx];
	//TDC[i] = (x5743Event->DataGroup[gridx]).TDC;
	...
	for (int j = 0; j < nADC; ++j)
	 {
	 double value;
	 if(i%2==0)	value = (x5743Event->DataGroup[gridx]).DataChannel[0][j];
	 else value = (x5743Event->DataGroup[gridx]).DataChannel[1][j];
	...
	//Double_t ypmin;
    	//n_raw_maxima[i] =  spectrum->Search(RawS[i],sigma_p,"",peak_amp);
	n_raw_maxima[i] = npeaks; //dummy
