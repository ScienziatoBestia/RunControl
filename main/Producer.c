...
void startDaqDT5743() {
/*************************************************************************************************************/  
  err_code |= CAEN_DGTZ_OpenDigitizer(pDaqSharedMemory->connectionParamsDT5743[0],pDaqSharedMemory->connectionParamsDT5743[1],
                                      pDaqSharedMemory->connectionParamsDT5743[2],pDaqSharedMemory->connectionParamsDT5743[3],&handleDT5743);

  // Set again the DCOffset and TriggerThreshold
  for(int i=0; i<MaxDT5743NChannels; i++) {
    err_code |= CAEN_DGTZ_SetChannelDCOffset(handleDT5743, i, pDaqSharedMemory->DCOffset[i]);
    err_code |= CAEN_DGTZ_SetChannelTriggerThreshold(handleDT5743, i, pDaqSharedMemory->TriggerLevel[i]);
  }
...
//
  ...
  printf(">> pSharedBuffer->head = %i pSharedBuffer->tail = %i \n", pSharedBuffer->head, pSharedBuffer->tail);
